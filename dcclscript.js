        ...
        for (let j = 0; j < dataArrays.length; j++) {
            if (dataArrays[j][i]) {
                if (j === 0) {
                    ...
                    row.push(dataArrays[j][i][27] || ''); // Column 32
                    row.push(dataArrays[j][i][33] || ''); // Column 34
                } else {
                    // For Meter 2, only keep the relevant columns
                    row.push('', dataArrays[j][i][27] || '', dataArrays[j][i][33] || ''); // Empty for Date, keep columns 32 and 34
                }
            } else {
